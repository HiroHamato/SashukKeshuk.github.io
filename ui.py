...
def post():
	...
	s_get = f"SELECT * FROM `articles` WHERE text2='{text2}'"
	try:
		add_line(s)
		l = get_lines(s_get)
		messagebox.showinfo(title='success', message=f"article with  id {l[0]['id']} was added successfully!")
...
def update():
	...
	id3 = Id.get()
	print(house)
	s = f"UPDATE `articles` SET text='{text}', text2='{text2}', house_type={houseT}, rooms='{rooms}', city='{city}', type={house} WHERE id={id3}"
	try:
		del_line(s)
...
def delete():
	id2 = Id2.get()
	q = f"DELETE FROM `articles` WHERE id={id2};"
...
canvas = Canvas(root, height=1800, width=980)
canvas.pack()
...
label2.pack()
textInput2 = Text(frame, bg='#000', fg='#fff', width='60', height='20', cursor='cross white')
textInput2.pack()
...
label1.pack()
textInput22 = Text(frame1, bg='#000', fg='#fff', width='60', height='20', cursor='cross white')
textInput22.pack()

label2 = Label(frame1, text='full description', bg='#000', fg='#fff', font=20)
label2.pack()
textInput21 = Text(frame1, bg='#000', fg='#fff', width='60', height='20', cursor='cross white')
textInput21.pack()
